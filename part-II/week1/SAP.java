...
public class SAP {
     ...
     * @param v
     * @param w
     * @return a common ancestor of v and w that in a shortest ancestral path;
     * -1 if no such path exists
     */
     ...
     * @param v
     * @param w
     * @return length of shortest ancestral path between 
     * any vertex in v and any vertex in w; -1 if no such path exists
     */
    ...
    private SAPProcessor cachedResult(int v, int w) {
        ...
        if (cache.containsKey(key)) {
            ...
            // we need to cache only for 2 consecutive calls, 
            //therefore we delete the result after the second call
            cache.remove(key);
            return p;
        }
    ...
    private SAPProcessor cachedResult(Iterable<Integer> v,
            Iterable<Integer> w) {
    ...
    private class SAPProcessor {
        ...
        private void process(BreadthFirstDirectedPaths a,
            ...
            for (int curAncestor : ancestors) {
                int dist = a.distTo(curAncestor) + b.distTo(curAncestor);
                if (dist < minDistance) {
                    minDistance = dist;
                    shortestAncestor = curAncestor;
                }
            }
